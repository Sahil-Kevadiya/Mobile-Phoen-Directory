...
mydb = connect(host = 'localhost',user = 'root',password = 'AbCd1928%' , database = 'phonedirectory' )
    ...
    def delete(self,dl):
        ...
        for i in con:
            if dl in i[0] or i[1] or i[2]:
                mycursor.execute("DELETE FROM contactlist WHERE name like '%" + dl + "%' or number1 LIKE '%" + dl + "%' OR number2 LIKE '%" + dl + "%'")
                mydb.commit()
